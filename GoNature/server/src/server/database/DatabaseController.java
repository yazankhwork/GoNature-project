...
public class DatabaseController {
		...
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
			...
			if (rs.next()) {
				if (rs.getString("password").equals(password)) {
					...
					return new String[] { "WRONG_PASSWORD", null, "NONE" };
			} else
	...
	public boolean saveBooking(Booking b) {
		...
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
			...
			pstmt.setString(8, b.getVisitorType());
		...
		try (PreparedStatement pstmt = connection.prepareStatement(wlQuery)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
					...
					b.setVisitorType(rs.getString("visitor_type"));
					list.add(b);
				}
			}
		} catch (Exception e) {
	...
	public boolean exitBooking(int bookingId) {
		String q = "UPDATE bookings SET status = 'Exited' " + "WHERE booking_id = ? AND status = 'Entered'";
	...
	public boolean confirmArrival(int bookingId) {
		try (PreparedStatement pstmt = connection
				.prepareStatement("UPDATE bookings SET status = 'Confirmed' WHERE booking_id = ?")) {
		...
		try {
			...
			try (PreparedStatement ps = connection.prepareStatement(getQ)) {
				...
				if (rs.next()) {
					...
					b.setPrice(price);
					saveBooking(b);
					connection.prepareStatement("DELETE FROM waitinglist WHERE waiting_id = " + waitingId)
							.executeUpdate();
					return true;
				}
			}
