	...
	public Booking getBookingById(int bookingId) {
		...
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				if (rs.next()) {
					return new Booking(rs.getInt("booking_id"), rs.getString("visitor_id"), rs.getString("park_name"),
							rs.getDate("visit_date").toLocalDate(), rs.getTime("visit_time").toLocalTime(),
							rs.getInt("visitors_count"), rs.getString("status"));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean claimWaitingSpots(int bookingId, int spotsToTake) {
		...
		try {
			if (spotsToTake == oldB.getVisitorsCount()) {
				...
				Booking newB = new Booking(0, oldB.getVisitorId(), oldB.getParkName(), oldB.getVisitDate(),
						oldB.getVisitTime(), spotsToTake, "Pending");
		...
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
			...
			if (rs.next()) {
				if (rs.getString("password").equals(password)) {
					...
					return new String[] { "WRONG_PASSWORD", null };
			} else
				return new String[] { "USER_NOT_FOUND", null };
		} catch (Exception e) {
			e.printStackTrace();
